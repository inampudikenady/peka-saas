...
## 31. Implemented v1 Components

The v1 implementation follows this specification through these boundaries:

- `KnowledgeService` remains the only retrieval dependency of `AIAnswerService`.
- `OpenAICompatibleLLMProvider` supplies embedding, generation, and streaming
  capabilities. The existing ingestion embedding adapter remains in place so the
  document pipeline is not disrupted.
- `PromptBuilder` uses prompt version `ai-answer-v2`, separates the system policy
  from user content, requests concise question-appropriate Markdown with grouped
  citations, applies deterministic citation IDs, deduplicates evidence, and
  reserves output capacity before admitting complete evidence blocks.
- `CitationValidator` removes hidden-reasoning tags as defense in depth, rejects
  unknown citations, and constructs citation responses from server-owned
  Knowledge Service metadata.
- `AIAnswerService` performs retrieval, evidence sufficiency checks, prompt
  construction, generation, citation validation, and one missing-citation retry.
- the synchronous and SSE transports are authenticated tenant routes over the
  same stateless domain pipeline.
- chat health is reported independently from ingestion and retrieval health.
- the tenant AI Assistant holds only the current, in-memory request and response.

The streaming transport buffers provider output until the final text has passed
reasoning suppression and citation validation. It then emits bounded `token`
events followed by validated citations. This deliberately prioritizes the
guarantee that invalid citations or hidden reasoning never reach the client over
displaying unvalidated provider deltas.

For native Ollama through `/v1/chat/completions`, the provider sends
`reasoning_effort: "none"` and `think: false`, consumes only the final content
channel, ignores reasoning fields, and applies tag suppression as a final
defense. A response that ends because of the provider length limit is rejected
as truncated rather than returned to a tenant.

The current Knowledge Service score is the vector store's cosine-similarity score.
`PEKA_AI_MIN_RETRIEVAL_SCORE` is therefore calibrated against that existing
contract. Score normalization or reranking is a future retrieval concern and is
not implemented in the AI layer.
