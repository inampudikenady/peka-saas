...
All ownership-bearing rows include `tenant_id` and all application lookups require
it. Tenant deletion remains cascading. Connector references on tenant knowledge
use `SET NULL`, because connectors are producers rather than owners. The
logical-document uniqueness constraint is `(tenant_id, document_key)` and version
uniqueness is `(document_id, content_hash)`. Document provenance records the
creating connector, most recent observing connector, and synchronization time.
