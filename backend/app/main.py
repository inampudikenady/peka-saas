...
from app.api.routes.tenant.ai_conversations import (
    router as tenant_ai_conversations_router,
)
...
app.include_router(
    platform_settings_router, prefix=settings.api_prefix, tags=["platform-settings"]
)
app.include_router(
...
app.include_router(
    tenant_users_router, prefix=settings.api_prefix, tags=["tenant-users"]
)
app.include_router(connectors_router, prefix=settings.api_prefix, tags=["connectors"])
app.include_router(
    platform_connectors_router, prefix=settings.api_prefix, tags=["platform-connectors"]
)
app.include_router(
    tenant_connectors_router, prefix=settings.api_prefix, tags=["tenant-connectors"]
)
app.include_router(
    tenant_ai_answer_router, prefix=settings.api_prefix, tags=["tenant-ai"]
)
app.include_router(
...
async def connector_maintenance_loop() -> None:
    while True:
        ...
        try:
            ConnectorService(
                ConnectorRepository(db), TenantRepository(db)
            ).maintenance()
        except Exception:
            db.rollback()
