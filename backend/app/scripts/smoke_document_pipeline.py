...
def _fixtures() -> dict[str, tuple[bytes, str]]:
    word = WordDocument()
    word.add_heading("Password Policy", level=1)
    word.add_paragraph("The service account password rotates every 90 days.")
    word_bytes = io.BytesIO()
    word.save(word_bytes)
    workbook = Workbook()
    sheet = workbook.active
    sheet.title = "Rotation"
    sheet.append(["Account", "Days"])
    sheet.append(["Linux service", 90])
    excel_bytes = io.BytesIO()
    workbook.save(excel_bytes)
    return {
        "smoke.txt": (
            b"PEKA smoke marker: rotate the Linux service password every 90 days.",
            "text/plain",
        ),
        "smoke.pdf": (
            _pdf("PEKA PDF smoke marker requires quarterly access review."),
            "application/pdf",
        ),
        "smoke.docx": (
            word_bytes.getvalue(),
...
def main() -> None:
    ...
    try:
        ...
        if tenant is None:
            raise RuntimeError("Tenant 'vitwo' does not exist")
        ...
        if admin is None:
            raise RuntimeError("VITWO has no active tenant administrator")
        connector_service = ConnectorService(
            ConnectorRepository(session), TenantRepository(session)
        )
        registration = connector_service.create_registration_token(
            tenant.id, admin, None
        )
        ...
        registered = client.post(
            ...
            json={
                ...
                "connector_name": "Document Smoke Connector",
        ...
        connector_headers = {
            "Authorization": f"Bearer {credentials['connector_secret']}",
        ...
        for filename, (content, mime_type) in _fixtures().items():
            ...
            metadata = {
                ...
                "relative_path": f"smoke/{filename}",
                "filename": filename,
                "mime_type": mime_type,
        ...
        while time.monotonic() < deadline:
            ...
            states = {
                item["document_id"]: item["ingestion_status"]
                for item in status_response.json()
            }
            if len(states) == 4 and all(
                value == "INDEXED" for value in states.values()
            ):
                break
            time.sleep(1)
        ...
        tenant_token = create_tenant_access_token(
            admin.id, admin.username or admin.email, tenant.id
        )
        tenant_headers = {"Authorization": f"Bearer {tenant_token}"}
        search = client.post(
            "/t/vitwo/api/v1/tenant/search",
            headers=tenant_headers,
            json={
                "query": "Linux service password rotation",
                "top_k": 8,
                "filters": {},
            },
        )
        ...
        if not results or any(
            item["document_id"] not in document_ids for item in results
        ):
            ...
            json.dumps(
                {
                    ...
                    "formats_indexed": sorted(_fixtures()),
        ...
        if connector_id is not None:
            ...
            if connector is not None:
                documents = list(
                    session.scalars(
                        select(Document).where(Document.connector_id == connector.id)
                    )
                )
