...
class KnowledgeService:
    ...
    def _validate_filters(self, tenant_id: UUID, request: SearchRequest) -> None:
        ...
        if connector_id is not None:
            connector = self.repository.session.scalar(
            ...
            if connector is None:
                ...
                raise KnowledgeFilterError(
                    "Connector filter is not available for this tenant"
                )
        if request.filters.document_id is not None:
            document = self.repository.get_document(
                tenant_id, request.filters.document_id
            )
            if document is None:
                logger.warning(
                ...
                raise KnowledgeFilterError(
                    "Document filter is not available for this tenant"
                )
            if connector_id is not None and document.connector_id != connector_id:
                raise KnowledgeFilterError(
                    "Document and connector filters do not match"
                )
            if (
            ...
            source_document = self.repository.session.scalar(
                select(Document)
                .where(
            ...
            if source_document is None:
                raise KnowledgeFilterError(
                    "Source filter is not available for this tenant"
                )
    ...
    def search(self, tenant_id: UUID, request: SearchRequest) -> SearchResponse:
        ...
        for hit in hits:
            ...
            chunk = self.repository.session.scalar(
            ...
            results.append(
                KnowledgeResult(
                    knowledge_id=f"document:{chunk.id}",
