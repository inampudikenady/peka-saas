...
class PlatformUserService:
    def __init__(
        self, users: PlatformAdminRepository, invites: PlatformAdminInviteRepository
    ) -> None:
        self.users = users
    ...
    def create(
        self, payload: PlatformUserCreate, actor: PlatformAdmin
    ) -> PlatformInvitationResponse:
        if self.users.exists_by_username(
            payload.username
        ) or self.users.exists_by_email(payload.email):
        ...
        try:
            ...
            invite, raw = self._new_invite(
                user.id, actor.id, PlatformAdminInvitePurpose.SETUP
            )
            self.users.commit()
        ...
        return PlatformInvitationResponse(
            user=user, setup_link=self._reset_link(raw), expires_at=invite.expires_at
        )

    def update(
        self, user_id: UUID, payload: PlatformUserUpdate, actor: PlatformAdmin
    ) -> PlatformAdmin:
        user = self.get(user_id)
        if (
            user.id == actor.id
            and user.role == PlatformAdminRole.PLATFORM_ADMIN
            and payload.role != PlatformAdminRole.PLATFORM_ADMIN
            and self.users.count_active_admins() <= 1
        ):
            raise PlatformUserError(
                "The last active platform administrator cannot remove their own admin role."
            )
    ...
    def set_active(
        self, user_id: UUID, active: bool, actor: PlatformAdmin
    ) -> PlatformAdmin:
        user = self.get(user_id)
    ...
    def password_reset(
        self, user_id: UUID, actor: PlatformAdmin
    ) -> PlatformInvitationResponse:
        user = self.get(user_id)
        try:
            invite, raw = self._new_invite(
                user.id, actor.id, PlatformAdminInvitePurpose.PASSWORD_RESET
            )
            self.users.commit()
        except Exception:
            self.users.rollback()
    ...
    def change_password(
        self, user: PlatformAdmin, current_password: str, new_password: str
    ) -> None:
    ...
    def _new_invite(
        self, user_id: UUID, actor_id: UUID, purpose: PlatformAdminInvitePurpose
    ):
        ...
        invite = PlatformAdminInvite(
            user_id=user_id,
            created_by_platform_admin_id=actor_id,
            purpose=purpose,
