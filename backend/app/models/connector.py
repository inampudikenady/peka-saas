...
class ManagedConnector(Entity):
    ...
    __table_args__ = (
        CheckConstraint(
            ...
            name="ck_managed_connectors_heartbeat_interval",
        ),
        CheckConstraint(
            "source_total >= 0 AND source_healthy >= 0 AND source_unhealthy >= 0 AND source_disabled >= 0",
            name="ck_managed_connectors_source_counts",
        ),
    ...
    status: Mapped[ManagedConnectorStatus] = mapped_column(
        Enum(ManagedConnectorStatus, name="managed_connector_status"), nullable=False
    )
    ...
    local_knowledge_store_status: Mapped[str | None] = mapped_column(
        String(32), nullable=True
    )
    knowledge_document_count: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0
    )
    knowledge_indexed_chunk_count: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0
    )
    last_knowledge_index_activity_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    consecutive_missed_heartbeats: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0
    )
    authentication_failure_count: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0
    )
...
class ConnectorRegistrationToken(Entity):
    ...
    __table_args__ = (
        Index(
            "ix_connector_registration_tokens_tenant_created", "tenant_id", "created_at"
        ),
    )
    ...
    intended_connector_name: Mapped[str | None] = mapped_column(
        String(255), nullable=True
    )
    expiration_event_recorded_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
...
class ConnectorHeartbeat(Entity):
    ...
    __table_args__ = (
        Index(
            "ix_connector_heartbeats_connector_received", "connector_id", "received_at"
        ),
    )
...
class ConnectorCapability(Entity):
    ...
    __table_args__ = (
        UniqueConstraint(
            "connector_id", "name", name="uq_connector_capabilities_connector_name"
        ),
    )
...
class ConnectorEvent(Entity):
    ...
    __table_args__ = (
        Index("ix_connector_events_connector_occurred", "connector_id", "occurred_at"),
        Index("ix_connector_events_tenant_occurred", "tenant_id", "occurred_at"),
    )
    ...
    registration_token_id: Mapped[UUID | None] = mapped_column(
        ForeignKey("connector_registration_tokens.id", ondelete="SET NULL"),
        nullable=True,
    )
    event_type: Mapped[ConnectorEventType] = mapped_column(
        Enum(ConnectorEventType, name="connector_event_type"), nullable=False
    )
