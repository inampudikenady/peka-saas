...
class Document(Entity):
    ...
    __table_args__ = (
        UniqueConstraint(
            "tenant_id", "document_key", name="uq_documents_tenant_document_key"
        ),
    ...
    connector_id: Mapped[UUID | None] = mapped_column(
        ForeignKey("managed_connectors.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
...
class DocumentVersion(Entity):
    ...
    storage_status: Mapped[StorageStatus] = mapped_column(
        Enum(StorageStatus, name="document_storage_status")
    )
    ingestion_status: Mapped[IngestionStatus] = mapped_column(
        Enum(IngestionStatus, name="document_ingestion_status"), index=True
    )
    ...
    format_detection_confidence: Mapped[float | None] = mapped_column(
        Float, nullable=True
    )
    format_detection_reason: Mapped[str | None] = mapped_column(
    ...
    chunking_started_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
...
class IngestionJob(Entity):
    ...
    __table_args__ = (
        ...
        Index(
            "uq_ingestion_jobs_active_version_stage",
            "version_id",
            "job_type",
            unique=True,
    ...
    job_type: Mapped[IngestionJobType] = mapped_column(
        Enum(IngestionJobType, name="ingestion_job_type")
    )
    state: Mapped[IngestionJobState] = mapped_column(
        Enum(IngestionJobState, name="ingestion_job_state"), index=True
    )
...
class DocumentChunk(Entity):
    ...
    __table_args__ = (
        UniqueConstraint(
            "version_id", "chunk_index", name="uq_document_chunks_version_index"
        ),
    )
...
class DocumentParsedSection(Entity):
    ...
    __table_args__ = (
        UniqueConstraint(
            "version_id",
            "section_index",
            name="uq_document_parsed_sections_version_index",
        ),
    )
...
class DocumentIdempotencyRecord(Entity):
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "connector_id",
            "idempotency_key",
            name="uq_document_idempotency_scope",
        ),
    )
    ...
    connector_id: Mapped[UUID] = mapped_column(
        ForeignKey("managed_connectors.id", ondelete="CASCADE"), index=True
    )
