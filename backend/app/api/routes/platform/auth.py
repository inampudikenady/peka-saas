...
from app.schemas.platform_auth import PlatformChangePasswordRequest, PlatformLoginRequest, PlatformPasswordResetRequest, PlatformTokenResponse, PlatformUserResponse
...
def reset_password(payload: PlatformPasswordResetRequest, service: PlatformUserService = Depends(get_platform_user_service)) -> Response:
    try:
...
def change_password(payload: PlatformChangePasswordRequest, user: PlatformAdmin = Depends(allow_platform_user), service: PlatformUserService = Depends(get_platform_user_service)) -> Response:
    try:
