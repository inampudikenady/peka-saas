...
export default function ConnectorsPage(){
  ...
  useEffect(()=>{if(user?.role!=="tenant_admin")return;void tenantApi.connectors(tenantSlug,true).then(setItems).catch(e=>setError(e.message));},[tenantSlug,user?.role]);
  ...
  return <TenantShell slug={tenantSlug} user={user} title="Connectors" adminOnly><div className="mb-6 flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-2xl font-semibold">Connectors</h2><p className="text-sm text-slate-500">Customer appliance communication and local source health.</p><p className="mt-1 text-xs text-slate-500">Connected means the connector is communicating with PEKA. It does not mean source data has been uploaded or synchronized.</p></div><Button asChild><Link href={`/t/${tenantSlug}/connectors/registration-tokens`}>Registration tokens</Link></Button></div>{error&&<Alert>{error}</Alert>}{items?<ConnectorInventory connectors={items} detailBase={`/t/${tenantSlug}/connectors`} canRetire={user.role==="tenant_admin"} onRetire={retire}/>:<p className="text-sm text-slate-500">Loading connectors…</p>}</TenantShell>;
}
